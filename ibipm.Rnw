...
To define the pgf $\Psi$, we introduce the following notation: given a continuous function $h:X\to\R$ and $\s\in \S$ (individuals' states $x_i$ are real numbers, and the state $s$ is within all possible states $S$), let
\[
h^\s=
\left\{
\begin{array}{cc}
...
If the function $h(x)$ is defined such that it corresponds to the probability that an individual of size $x$ dies and has no offspring over the next year, then $h^\s$ corresponds to the probability that a population in state $\s$ goes extinct in the next year. The corresponding probability-generating functional is defined by

\begin{equation}\label{*}
\Psi(h)(x)=\int h^\s \, m(x, d\s).
\end{equation}
Given an individual in state $x$, $\Psi(h)(x)$ corresponds to the expected value of $h^\s$.  This requires integrating over all possible
states $s$ that the population may take on in the next time step due to the different possible contributions of the individual in state $x$.
...
\Psi(h_0)(x)=\int 1 \times m(x, d\s)= \P[\s_1=0|\s_0=(1;x)]
\end{equation}
Secondly, we clam that the t-fold composition of $\Phi$ to $h_0$,
\[
\Psi^t(h_0)=\underbrace{\Psi(\Psi(\dots \Psi}_{\mbox{$t$ times}}(h_0)\dots))
\]
...
\section*{Appendix: Extinction probabilities in a time varying environmnent}


Let $m_0(x,d\s),m_1(x,d\s),m_2(x,d\s),\dots$ be a sequence of probability measures defining the population update rules at times $0,1,2,\dots.$. For each of these probability measures, define the pgf 
\[
\Phi_t(h)(x)=\int h^\s m_t(x,d\s)
\]
We claim that for $t\ge 0$ and $\tau\ge 0$
\begin{equation}\label{claim}
\Psi_{t}(\Psi_{t}(\dots \Psi_{t+\tau}(h_0)))(x)
\end{equation}
...
\begin{equation}\label{star}
\Psi_t(h_0)(x)=\int 1\times m_t(x, d\s)= \P[\s_{t+1}=0|\s_t=(1;x)]
\end{equation}
Now suppose that equation \eqref{claim} holds for some $\tau\ge 0$ and all $t$; we will show it holds $\tau+1$ and all $t$. Fix $t\ge 0$ and $\tau \ge 0$. Assume $\s_t=(1;x)$.  On the event that $\s_{t+1}=(n_1, \dots, n_k;x_1, \dots, x_k)$ is the population state at time $t+1$, extinction occurs by time $t+\tau+1$ only if each of the lineages of the $n_1+\dots +n_k$ individuals go extinct in the next $\tau$ time steps. As the fates of these lineages are independent of one another, it follows that
\[
\begin{aligned}
\P[s_{t+\tau+1}=0|\s_{t}=(1; x), \s_{t+1}=(n_1, \dots, n_k;x_1, \dots, x_k)]&= \prod_{i=1}^k \P[\s_{t+\tau+1}=0|\s_{t+1}=(1; x_i)]^{n_i}\\
...
\end{aligned}
\]
where the second equality follows from our inductive hypothesis. By the law of total probability and conditioning over all possible states at time $t+1$, we get
\[
\begin{aligned}
\P[s_{t+\tau+2}=0|\s_t=(1;x)]&=\int \Psi_{t+1}(\Psi_{t+2}(\dots \Psi_{t+\tau+1}(h_0)))^{\s} \, m_t(d\s, x)\\
