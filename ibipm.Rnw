...
\end{equation}
Now suppose that equation \eqref{claim} holds for some $\tau\ge 0$ and all $t$; we will show it holds $\tau+1$ and all $t$. Fix $t\ge 0$ and assume $\s_t=(1;x)$.  On the event that $\s_{t+1}=(n_1, \dots, n_k;x_1, \dots, x_k)$ is the population state at time $t+1$, extinction occurs by time $t+\tau+1$ only if each of the lineages of the $n_1+\dots +n_k$ individuals go extinct in the next $\tau$ time steps. As the fates of these lineages are independent of one another, it follows that
\[
\begin{aligned}
