...
matplot(xs,t(out[times,]),xlim=c(alpha,beta),type="l",lty=1,xlab="size x",ylab="extinction probabilities",bty="n",lwd=2)
h=AE()
...
boxplot(log10(stuff),notch=FALSE,names=sample.sizes,col="lightgray",range=0,xlab="initial population abundance",ylab=expression(paste(log[10]," extinction probability")),ylim=c(min(log10(c(stuff,stuff2))),max(c(log10(stuff),log10(stuff2)))))
