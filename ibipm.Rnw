...
To define the pgf $\Psi$, we introduce the following notation: given a continuous function $h:X\to\R$ and $\s\in \S$ (individuals' states $x_i$ are real numbers, and the stats $s$ is within all possible states $S$), let
\[
h^\s=
\left\{
\begin{array}{cc}
