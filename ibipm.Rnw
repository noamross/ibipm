...
\begin{enumerate}
\item  Matrix models or integral projection models (IPMs) are commonly used to study the dynamics of structured populations, where some discrete or continuous trait influences survival, growth, or reproduction. When the population's size is small, as is often the case for a threatened species or a potentially invasive species arriving in a novel habitat, extinction risk may be substantial due to demographic stochasticity.
...
image(xs, xs, t(outer(xs, xs, Growth)), xlab = expression(size[t]),
      ylab = expression(size[t + 1]), col = inferno(24)[8:24])
...
boxplot(log10(stuff2), notch = FALSE, names = sample.sizes, col = "pink", range = 0,
    add = TRUE)
legend("bottomleft", c("in 10 years", "in 5 years"), pch = 22, pt.bg = c("pink",
...
legend("bottomleft", c("in 10 years", "in 5 years"), pch = c(20, 20), col = c("pink",
    "lightgray"), cex = 1.25, bty = "n")
