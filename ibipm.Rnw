...
# Inputs: N vector of length $n$ which specifies the initial number of individuals in each size bin, and T the length of
# the time to look things over. Output:a vector of length T+1 corresponding to the probabilities of extinction by time t
# or sooner
PVA <- function(N, Tf) {
    hs <- Psi.iterate(Tf)
    output <- numeric(Tf + 1)
...
matplot(xs, t(out[times, ]), xlim = c(alpha, beta),type = "l", lty = 1,
        xlab = "size x", ylab = "extinction probabilities", bty = "n", lwd = 2)
h <- AE()
lines(xs, h, type = "l", lwd = 5, xlim = c(alpha, beta), col = rgb(0, 0, 0, 0.5))
legend("bottomleft","A",bty="n",cex=1.25)


increase.factor<-3
phi2<-function(x,s)exp(increase.factor*Fecundity(x)*(s-1))

Psi2<-function(h){
  h.new<-(s.vec*(t(g.mat)%*%h)+1-s.vec)*(1-flowering.vec+flowering.vec*phi2(xs,e.vec%*%h))
  return(h.new)
}
Psi2.iterate<-function(Tf){
hs<-matrix(0,Tf+1,n)
for(t in 1:T){
hs[t+1,]<-Psi2(hs[t,])
}
...
legend("bottomleft","B",bty="n",cex=1.25)
@
