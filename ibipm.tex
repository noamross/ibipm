...
\includegraphics[width=\maxwidth]{components-1} \caption[The demographic kernels for Plateau Yellow Miner's Candle with the corresponding data from Salguero et al]{The demographic kernels for Plateau Yellow Miner's Candle with the corresponding data from Salguero et al. 2012.}\label{fig:components}
\end{figure}
...
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}
\includegraphics[width=\maxwidth]{compare-1} \caption[Extinction probabilities as a function of time for populations with ]{Extinction probabilities as a function of time for populations with $100$ individuals of size $x=1$, and populations with $5$ or $8$ individuals of size $x=60$.}\label{fig:compare}
\end{figure}
...
\vskip 0.1in
\fbox{\parbox{\textwidth}{\textbf{Fundamental Property 3}: Let $X_1$,$X_2$,\dots,$X_n$ be $n$ independent, random variables with pgfs $\Psi_1$, $\Psi_2$, \dots, $\Psi_n$. If $Y=X_i$ with probability $p_i$ where $p_1+p_2+\dots+p_n=1$, then the pgf of $Y$ is $\sum_{i=1}^n p_i \Psi_i$.}}
\vskip 0.1in
