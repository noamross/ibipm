# This script creates functions for the survival, growth, and fecundity kernels
# used to create the indiviudal-based-integro-projection model.

# Adapted from code of Jessica Metcalf, Sean McMahon, Rob Salguero-Gomez, and
# Eelke Jongenans from an ESA workshop in 2012.  Their code, in turn, is
# adapted from code by Johan Dahlgren from the publication Nicole et al. 2011. Interdependent effects of habitat
# quality and climate on population growth of an endangered plant. J Ecol 99 1211-1218.  The IPM is of the form
# N[y,t+1]=Integal of s[x]*G[y,x]+F[y,x]dx where s[x] is the probability of surviving for an individual of size x
# G[y,x] is the probability density for growing to size y given individual is of size x and has survived F[y,x] is
# the density of offsrping of size y produced by individuals of size x
...
recruit.size = suppressWarnings(fitdistr(data$sizeNext[is.na(data$size)], "gamma")
