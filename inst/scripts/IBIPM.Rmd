...
To create this probability generating functional numerically, we discritize the size interval $[\alpha,\beta]$ using $n$ equal sized intervals of width $dx=(\beta-\alha)/n$. **NEED TO MAKE SURE EVERYTHING IS CONSISTENT WITH MIDPOINT RULE**
...
Intuitively, this figure illustrates that (i) extinction probability descreases if the size of one individual initially in the popoulation is larger, and (ii) extinction probabilities increase over time. Furthermore, this figure illustrates that $E_t(x)$ are approaching limiting extinction probabilities $E_{\infy}(x)$ which are less than one. This stems from the fact that the dominant eigenvalue of the meanfield IPM is greater than one. The following function computes this asymptotic probability of extinction by iterating until a tolerance condition is met.  
