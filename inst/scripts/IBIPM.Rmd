...
Recall that pgf for a Poisson distribution with mean $\lambda$ is $\phi(s)=\exp(\lambda(s-1))$ where $s$ is a dummy variable. Hence, the pgf for the number of offspring of an individual of size $x$ is given by 
```{r}
...
\[
s(x)=\frac{1}{1+\exp(-a-bx)}
\]
and its derivative with respect to the parameter $b$ is 
\[
\frac{\partial s}{\partial b}(x)=\frac{x\exp(-a-bx)}{(1+\exp(-a-bx))^2}
\]
Hence, the deriviative of $\Psi$ with respect to $b$ is given by 
\[
\frac{\partial \Psi}{\partial b}=\left(-\frac{\partial s}{\partial b} + \frac{\partial s}{\partial b}\int h(y)G(y,x)dy\right)\phi(x,\int h(y)e(y)dy).
\]
...
\]
**Need to implement these formulas and then create plots showing how sensitivities of extintion probabilities depend on size and time**
