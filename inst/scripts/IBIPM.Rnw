...
<<boring-plots, fig.width=7, fig.height=5,echo=FALSE,fig.cap="The demographic kernels for Austrian Dragonhead with the corresponding data from Nicole et al. (2011).">>=
rm (list = ls ())
load("IPMs.Rdata")
...
<<IBIPM-plots3, fig.width=6, fig.height=4,echo=FALSE,fig.cap="Extinction probabilities as a function of time for populations of different abundances and size distributions.">>=
T=75
N=rep(0,n)
N[1]=100
out1=PVA(N,T)
...
<<IBIPM-plots4,cache=TRUE, fig.width=7, fig.height=4,echo=FALSE,fig.cap="Extinction probabilities for populations of different abundance sampled for the size distribution data. For each population abundance $N$, extinction probablities by year $5$ and year $50$ were calculated for $500$ samples of size $N$. In A, extinction probability is plotted as a box plot for different population abundances and different time frames. In B, extinction probability is plotted against the mean size of an individual for a population abundance $N=25$.">>=
...
boxplot(log10(stuff),notch=FALSE,names=sample.sizes,col="lightgray",range=0,xlab="population abundance",ylab=expression(paste(log[10]," extinction probability")),ylim=c(min(log10(stuff2)),max(log10(stuff))))
...
% Acknowldegements: Thanks to Jacob Moore for several excellent suggestions including the addition of Figure 4B.
