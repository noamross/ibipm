...
When the random variables are identically, distributed, the following generalization of this fundamental property of pgfs will be useful:
...
Alternatively, we can use the actual population data from 2001 to see how extinction risk over different time frames, $5$ or $50$ years, depends on the size of a random subpopulation. This figure illustrates that $\log$ extinction risk decreases linearly with population size where the variation around this mean trend stems from the random sampling of individuals.
...
1. Temporally varying environments - relatively straightforward to implement; perhaps outline the idea. 
